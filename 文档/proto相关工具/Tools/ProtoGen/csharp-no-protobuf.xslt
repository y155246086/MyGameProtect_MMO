...
// Note: requires additional types generated from: <xsl:value-of select="."/></xsl:template>
  ...
  <xsl:template match="FieldDescriptorProto[label='LABEL_REPEATED']">
    ...
    public int Get_<xsl:call-template name="pascal"/>Count()
    {
      return <xsl:value-of select="$field"/>.Count;
    }
    public <xsl:value-of select="$type"/> Get_<xsl:call-template name="pascal"/>ByIndex(int index)
    {
      return <xsl:value-of select="$field"/>[index];
    }
  </xsl:template>
