...
namespace Common
{
	public class BigEndianTransfer
	{
		...
		public static byte[] GUIDToBytes(byte[] guid)
		{
			//TODO:
			byte[] convertedGuid = new byte[16];
			convertedGuid[0] = guid[3];
			convertedGuid[1] = guid[2];
			convertedGuid[2] = guid[1];
			convertedGuid[3] = guid[0];
			convertedGuid[4] = guid[5];
			convertedGuid[5] = guid[4];
			convertedGuid[6] = guid[7];
			convertedGuid[7] = guid[6];
			Array.Copy(guid, 8, convertedGuid, 8, 8);
