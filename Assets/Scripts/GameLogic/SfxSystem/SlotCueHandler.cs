...
public class SlotCueHandler : MonoBehaviour
{
    ...
    private Dictionary<string, List<KeyValuePair<int, string>>> fxList;
    private SfxHandler handler;

    void Awake()
    {
        fxList = new Dictionary<string, List<KeyValuePair<int, string>>>();
    ...
    public void SetFxList(int id, string bone_path, string index)
    {
        if (fxList.ContainsKey(bone_path))
        {
            // to do 通过判断删除其他的应该被删除的
            // 比较列表的每一项
            foreach (KeyValuePair<int, string> item in fxList[bone_path])
            {
        ...
        if (!fxList.ContainsKey(bone_path))
            fxList.Add(bone_path, new List<KeyValuePair<int, string>>());

        fxList[bone_path].Add(new KeyValuePair<int, string>(id, index));
    }
