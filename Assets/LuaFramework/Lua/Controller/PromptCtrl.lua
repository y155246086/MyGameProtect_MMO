...
function PromptCtrl.TestLogin()
    local login = login_pb.CreateRoleReq();
    login.username = 'username111';
	login.password = '4545';
    local msg = login:SerializeToString();
    ----------------------------------------------------------------
    local buffer = ByteBuffer.New();
	--buffer:WriteLong(100);
	--buffer:WriteLong(101);
	--buffer:WriteInt(Protocal.LoginGame);
    buffer:WriteBuffer(msg);
    networkMgr:SendMessageForHttp(buffer,Protocal.P_CG_GAME_CREATE_ROLE);
end
--测试发送SPROTO--
